...
\begin{itemize}
    ...
    \item \texttt{insert(u)} agggiunge un elemento $u \in U$ a $S$
...
Si ricorando le variabili $n$ rappresentate il numero di elementi di $S$, $N = |U|$, $m$ numero primo compreso $N \leq m \leq 2N$. Si fa uso della tecnica di \emph{doubling-halving}

\input{algorithms/doubling_halving.tex}
Si ottiene tempo $O(1)$ ammortizzato per eliminazione e inserimento, poiché il re-hasing avviene solo quando la dimensione raddoppia.
