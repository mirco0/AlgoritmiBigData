...
Un algoritmo banale consiste nel calcolare il valore Hash di ogni elemento (quindi fare uso di \emph{bucket}) nella Stream è prendere una porzione di \emph{bucket}. Segue un esempio 
...
\end{align*}
Si giustificano i calcoli. Le query $d$ vengono campionate con probabilità $\frac{1}{10}$, ce ne sono due, la probabilità di campionarle entrambe è $\frac{1}{100}$ per $d$ query allora $\frac{d}{100}$, $\frac{18d}{100}$ vengono campionati una sola volta ($\left(\frac{1}{10} \cdot \frac{9}{10}\right) d$ volte). 
...
Sia $Q$ l'insieme delle query, $I$ l'insieme degli \emph{ID utente}. Fissando una data query $q$ si definisce la funzione $x(v,q)$ come il numero di \emph{occorrenze $q$} da $u$ nella Stream. Inoltre si definisce la variabile aleatoria $X_S(V,q)$ come il numero di occorrenze dall'utente $V$ (variabile)in $S$. Nell'algoritmo \ref{alg:user_sample} gli ID degli utenti $V$ è una variabile aleatoria uniforme.
...
\end{proof}
Nel caso in cui una finestra $N$ non possa essere salvata in memoria è necessario fornire un'approssimazione per il valore della funzione $\#1(I,N,k)$. Si fa uso del metodo \emph{DIGM}
\subsubsection*{DIGM (Datar, Gioinis, Indikm, Motwani)}
Il metodo \emph{DIGM} funziona con qualsiasi distribuzione su Stream lo spazio utilizzato è $O(\log 2N)$ bit per Stream, e non fornisce mai l'approssimazione più del $50\%$ del valore effettivo.

Più precisamente (dalle note di Prezza) DIGM usa $O(\varepsilon^{-1} \log^{2}m)$ spazio fornendo una $(1+ \varepsilon)$ approssimazione. Ne segue una descrizione del funzionamento, sia $B = \lceil 1 / \varepsilon \rceil$, i bit della Stream vengono raggruppati in gruppi $G_1,G_2,\dots,G_t$ che devono soddisfare le seguenti condizioni 
\begin{enumerate}
...
Si procede valutando il \emph{fattore di approssimazione} di \emph{DISM}.
%TODO: creare figura
...
\begin{enumerate}
    ...
    si fa uso di DIGM per la stima del numero di bit \textit{1} nella rappresentazione binaria di ogni intero. Sia $c_i$ il conto stimato per l'\textit{i-esimo} bit, la somma è $\sum_{i=0}^{m-1}c_{i}2^{i}$
