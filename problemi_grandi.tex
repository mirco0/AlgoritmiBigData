...
\begin{definition}[Similarità tra firme]
    ...
    \]
    In modo pù formale è possibile definire una variabile aleatoria $Z$ che assume i seguenti valori
    \[
        Z = \begin{cases*}
...
\vspace{1em} Sia $C_j$ una variabile aleatoria definita come segue \[
    ...
    \end{cases*}
\]
sia inoltre $x_j$ la variabile che conta il numero di elementi dell'utente $j$ in $U$. $\E{|S|} = \sum_j{x_j\cdot \Pr\left[C_j = 1\right]} = \frac{1}{10}\cdot \sum_{j}{x_j} = \frac{|U|}{10}$. È da notare che la varianza può esssere molto grande, si presenta quindi una soluzione generale. 
...
\begin{enumerate}
    \item si esegue l'hash della chiave in $b$ \emph{buckets}
...
\begin{enumerate}
    \item si esegue l'hash di ogni chiave in un grande numero $b$ di \emph{buckets}
...
Si suppone di voler mantere in memoria un campione di $S$ di esattamente $s$ elementi, supponendo di aver visto $n$ elementi al tempo $n$, allora ogni tuopla deve essere nel campione con probabilità $\frac{1}{n}$. Si presenta l'algoritmo di Reservoir-sampling
...
\begin{enumerate}
    ...
    \item se l'elemtento è scelto per l'inserimento si sceglie un elemento $x \in_u S$ da rimuovere con probabilità $\frac{1}{s}$.
\end{enumerate}

\begin{theorem}
    Per ogni $s\geq 1$ e per ogni tempo $n\geq s$ l'algoritmo Reservoir Sampling ha la seguente proprietà: dopo $n$ passi $S$ contiene ogniuno degli $n$ elementi con la stessa probabilità
\end{theorem}
\begin{proof}
    Caso Base:
...
Sia $I$ una Stream di bit infiita, con una finestra di lunghezza $N$. Si vuole calcolare il valore della funzione $\#1(I,N,k)$ che rappresenta il numero di \emph{`1'} negli ultimi $k$ bit ($k\leq N$). Una soluzione banale consiste nel memoriazzare gli ultimi $N$ bit
\begin{theorem}
...
Il meotodo \emph{DIGM} funziona con qualsiasi distribuzione su Stream lo spazio utilizzato è $O(\log 2N)$ bit per Stream, e non fornisce mai l'approssimazione più del $50\%$ del valore effettivo.

Più precisamente (dalle note di Prezza) DIGM usa $O(\varepsilon^{-1} \log^{2}m)$ spazio fornendo una $(1+ \varepsilon)$ approssimazione. Ne seugue una descrizione del funzionamento, sia $B = \lceil 1 / \varepsilon \rceil$, i bit della Stream vengono raggruppati in gruppi $G_1,G_2,\dots,G_t$ che devono soddisfare le seguenti condizioni 
\begin{enumerate}
...
\]
in cui, appunto, $G_{j_1},\dots,G_{j_s}$ sono tutti i gruppi contenenti $2^i$ bit \textit{1}. Ogni gruppo è una coppia \textit{(sx,dx)} che rappresenta l'inizio e la fine del gruppo. Per ogni lista collegata $\ell_i$ viengno salvate la testa, la coda, numero di bit contenuti e numero di bit \textit{1} contenuti nei suoi gruppi. 
