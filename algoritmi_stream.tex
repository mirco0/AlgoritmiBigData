...
Qual è la probabilità che lo stimatore $\tilde{f_y} > f_y + \varepsilon m$, si ricorda che lo stimatore $\tilde{f_y}$ è definito come il minimo fra tutte le righe, se il minimo supera  $f_y + \varepsilon m$ allora tutte le righe lo superan, e poiché le funzioni hash $h_j$ sono scelte indipendentemente
\[
...
\vspace{1em}\noindent
Una soluzione banale consiste nel salvare $S$ in una tabella hash $T$, per ogni elemento $x$ viene calcolata l'hash di $k_(x)$, successivamente verificata con la tabella $T$.
...
Mandando $m$ palline in $n$ \emph{bucket} con la stessa probabilità per ogniuno, qual è la probabilità che un \emph{bucket} riceva almeno una pallina. 
...
\] e poiché le $k$ funzioni hash sono mutualmente indipendenti e $x$ viene accettato se tutti i \emph{bucket} contengono 1. La probabilità dei falsi positivi è $\left( 1 - e^{-\frac{k\cdot m}{n}}\right)^k$.
...
Il problema del conteggio consiste nel, data una Stream sequenza di elementi scelti da un universo $U$, di dimensione $N$, mantenendo un contatore $d$ di elementi distrinti visti fino ad ora.
...
In cui in una fase preliminare viene scelta una funzione hash $h$ che mappa ogniuno degli $N$ elementi di $U$ in almeno $\log_2(N)$ bit 
\[
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item $\Pr[X_r = 1] = 1 - (1-1-\frac{1}{2^r})$ e $\Pr[X_r = 0] = \left(1-\frac{1}{2^r}\right)^d$ (dove $d$ è il vero numero di elementi distinti visti)
    \end{itemize}
...
\end{itemize}
\noindent
Si mostra un esempio per il numero \emph{sopresa}
