Iniziamo dando qualche definizione preliminare che permetteranno di poter analizzare al meglio gli algoritmi delle sezioni successive

\subsection{Spazi di campionamento e funzioni di probabilità}
\begin{definition}[Spazio di campionamento]
    Uno \emph{spazio di campionamento} $\Omega$ è un insisme di elementi che descrivono gli esiti di un esperimento di interesse
\end{definition}
\vspace{1em}
...
\begin{definition}[Funzione di probabilità]
    ...
    \begin{itemize}
        ...
        \[
            E_1 \cap E_2 = \emptyset \quad \Pr{[E_1 cap E_2]} = 0 
        \]
    ...
    \end{itemize}
\end{definition}
Da questi punti si arriva a un risultato noto, conosciuto come \emph{Union-buond} applicabile a una sequenza di eventi finiti o numerabili
    \[
    ...
    \begin{definition}[Spazio di probabilità]
        ...
        \begin{itemize}
            ...
            \item Una famiglia di insiemii $F$ che rappresenta tutti i possibili eventi $E \in F, E \subseteq \Omega$
...
\begin{definition}[Probabilità condizionata]
    ...
    \end{align*}
\end{definition}
È possibile calcolare la probabilità di un evento mediante le probabilità condizionate di una serie di eventi  disgiuti, i quali presi complessivamente formano lo \emph{spazio di campionamento}. Arriviamo a un altro risultato importante
\begin{theorem}[Teorema delle probabilità totali]
    Siano $E_1,E_2,\dots,E_n$ eventi mutualmente disgiuti tali per cui $\bigcup_{i=1}^{n} E_i = \Omega$. La probabilità di un evento arbitrario $E \subset \Omega$ può essere espressa come
    \begin{align*}
        \Pr{[E]} &= \sum_{i=1}^{n}\Pr{[E_i \cap E]}\\
        &= \Pr{[E|E_1]}\cdot\Pr{[E_1]} + \dots + \Pr{[E|E_m]}\cdot\Pr{[E_m]}\\
        &= \sum_{i=1}^{n}\Pr{[E|E_i]}\cdot\Pr{[E_i]}
    \end{align*}
\end{theorem}

\begin{theorem}[Teorema di Bayes]
    Siano $E_1,E_2,\dots,E_n$ eventi disgiuti tali per cui $\bigcap_{i=1}^{n} E_i = \Omega$, la probabilità condizionata di $E_i$ dato un evento arbitratio $E$ può essere espressa 
    \[
...
\begin{definition}[k-indipendenza]
    ...
    \begin{definition}[Valore atteso]
        ...
        \[
            \E{X} = \sum_{i=0}i\Pr\left[X=i\right]
        \]
        $i$ è definita su tutti i valori che $X$ può assumere
    \end{definition}
